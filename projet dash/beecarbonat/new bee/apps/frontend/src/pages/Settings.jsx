...
export default function Settings() {
  ...
  const [newKeyTier, setNewKeyTier] = useState('PUBLIC_READ');
  ...
  const handleCreateApiKey = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewKeyName('');
        toast.success(`API Secret Key generated: ${newKey.rawKey}. Save this, it will not be shown again!`, { duration: 8000 });
      } else {
              ...
              <div className="bg-zinc-900 border border-zinc-800 p-6 space-y-6">
                ...
                <div className="flex gap-2">
                  ...
                  <select
                    ...
                    className="bg-zinc-950 border border-zinc-800 text-zinc-300 px-3 py-2 text-xs font-mono rounded-sm outline-none"
                  >
                    <option value="PUBLIC_READ">Public Read</option>
                    <option value="PUBLIC_WRITE">Public Write</option>
                    <option value="PARTNER">Partner (High limit)</option>
                  </select>
                  <button
                    type="button"
                  ...
                  {apiKeys.map((key) => (
                    <div key={key.id} className="flex justify-between items-center bg-zinc-950 border border-zinc-800 p-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-[10px] font-mono text-zinc-500">Key: <code className="text-brand-orange font-semibold">{key.prefix}</code> • Tier: {key.tier} • Created: {new Date(key.createdAt || key.created).toLocaleDateString()}</p>
                      </div>
                      <button
                        type="button"
