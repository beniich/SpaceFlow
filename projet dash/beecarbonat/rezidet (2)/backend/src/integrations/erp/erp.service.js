...
class ERPService {
  ...
  async syncConnection(connectionId, options = {}) {
    ...
    try {
      ...
      if (connection.syncAssets) {
        try {
          const fetchAssetsBreaker = createCircuitBreaker(connector.fetchAssets.bind(connector));
          const erpAssets = await fetchAssetsBreaker.fire();
      ...
      if (connection.syncInvoices) {
        try {
          const fetchInvoicesBreaker = createCircuitBreaker(connector.fetchInvoices.bind(connector));
          const invoices = await fetchInvoicesBreaker.fire();
  ...
  async pushWorkOrderToERP(connectionId, workOrder) {
    ...
    const pushBreaker = createCircuitBreaker(connector.pushWorkOrder.bind(connector));
    const result = await pushBreaker.fire({
      ...workOrder,
