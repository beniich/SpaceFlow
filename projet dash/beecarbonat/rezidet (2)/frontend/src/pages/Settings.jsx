...
export default function Settings() {
  ...
  const [apiKeys, setApiKeys] = useState([
    { id: 'key-1', name: 'SAP S/4HANA Connector', prefix: 'cafm_sap_live_...', created: '2026-04-10', status: 'ACTIVE' },
    { id: 'key-2', name: 'Google Workspace OAuth Client', prefix: 'cafm_gws_prod_...', created: '2026-06-15', status: 'ACTIVE' }
  ]);
  ...
  const [alertConfig, setAlertConfig] = useState({
    ...
    telegramChatId: '@cafm_sre_alarms',
    hvacCriticalTemp: 28,
  ...
  const handleSaveAll = (e) => {
    e.preventDefault();
    setSaving(true);
    const saveToast = toast.loading('Saving system configuration changes...');
    setTimeout(() => {
      setSaving(false);
      toast.success('System parameters permanently committed to BEECARBONAT Database.', { id: saveToast });
    }, 1200);
  };
  ...
  const handleCreateApiKey = (e) => {
    ...
    const newKey = {
      id: `key-${Date.now()}`,
      name: newKeyName,
      prefix: `cafm_gen_${Math.random().toString(36).substr(2, 5)}_...`,
