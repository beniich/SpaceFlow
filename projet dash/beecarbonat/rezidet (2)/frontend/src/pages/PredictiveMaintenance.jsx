...
export default function PredictiveMaintenance() {
  ...
  // Compute predictive risk scores for assets dynamically (initial state)
  const [scoredAssets, setScoredAssets] = useState([]);
  ...
  const analyzeAsset = async (assetId) => {
    ...
    try {
      const { data } = await api.get(`/ai/health/${assetId}`);
      // Mettre à jour l'asset ciblé avec les vraies prédictions de l'IA
      setScoredAssets(prev => prev.map(a => {
        if (a.id !== assetId) return a;
        return {
          ...a,
          riskScore: 100 - data.score,
          riskCategory: (100 - data.score) >= 80 ? 'CRITICAL' : (100 - data.score) >= 50 ? 'WARNING' : 'MODERATE',
          timeHorizon: data.predictedFailure,
          riskFactor: data.anomalies?.[0]?.message || 'Aucune anomalie critique',
          aiAnalyzed: true
        };
      }));
      toast.success('Analyse IA SRE terminée');
    } catch (err) {
        ...
        {filteredAssets.map((asset) => (
          ...
          >
            ...
            <div className="pt-3 border-t border-zinc-800 flex items-center justify-between gap-2 flex-wrap">
              <Link
                to={`/assets/${asset.id}`}
                className="flex items-center gap-1 text-[11px] font-bold text-zinc-400 hover:text-cyan-400 transition"
              >
                Détails <ArrowRight className="w-3 h-3" />
              </Link>
              <button
                onClick={() => analyzeAsset(asset.id)}
                disabled={analyzing === asset.id || asset.aiAnalyzed}
                className="px-2 py-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-[11px] font-mono rounded flex items-center gap-1 transition disabled:opacity-50"
              >
                <Activity className="w-3 h-3" />
                {analyzing === asset.id ? 'Analyse...' : asset.aiAnalyzed ? 'Analysé' : 'IA'}
              </button>
              <button
                onClick={() => handleCreateUrgentWO(asset)}
                disabled={creatingWO === asset.id}
                className="px-2 py-1 flex-1 bg-zinc-950 hover:bg-zinc-800 border border-zinc-700 text-zinc-200 text-[11px] font-bold uppercase flex items-center justify-center gap-1 transition disabled:opacity-50"
              >
