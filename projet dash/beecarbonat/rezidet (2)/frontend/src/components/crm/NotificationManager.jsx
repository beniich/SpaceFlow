...
export default function NotificationManager() {
  ...
  const handleEvent = useCallback(({ type, data }) => {
    ...
    // Refresh queries
    if (['deal_won', 'new_deal'].includes(type)) queryClient.invalidateQueries({ queryKey: ['crm-deals'] });
    if (type === 'new_contact') queryClient.invalidateQueries({ queryKey: ['crm-contacts'] });
    if (['deal_won', 'kpi_threshold'].includes(type)) queryClient.invalidateQueries({ queryKey: ['crm-dashboard'] });
  }, [queryClient]);
